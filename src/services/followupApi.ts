// Serviço para integração com a API de Follow-up
...
export interface DetalheFollowUp {
  id_lead: string;
  nome_lead: string;
...
try {
  ...
  console.error('Erro na configuração da API de Follow-up:', error);
...
class FollowupApiService {
  ...
  async getEstatisticasPorDia(): Promise<ApiResponse<EstatisticasPorDia[]>> {
    return this.request<EstatisticasPorDia[]>('/followup/estatisticas-por-dia', {
      method: 'GET',
    });
  }

  async getDetalhesPorData(data: string): Promise<ApiResponse<DetalheFollowUp[]>> {
    return this.request<DetalheFollowUp[]>(`/followup/detalhes-por-data?data=${data}`, {
      method: 'GET',
    });
  }
}
