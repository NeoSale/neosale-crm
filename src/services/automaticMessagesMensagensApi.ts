...
try {
  ...
  console.error('Erro na configuração da API de Automatic Messages Mensagens:', error);
  // Em caso de erro, usar uma URL que causará erro explícito
  API_BASE_URL = '';
}

export interface AutomaticMessagesMensagem {
  id: string;
  nome?: string;
...
export interface AutomaticMessagesMensagemForm {
  nome?: string;
  intervalo_numero: number;
  intervalo_tipo: string;
  texto_mensagem: string;
...
class AutomaticMessagesMensagensApiService {
  ...
  async getMensagens(): Promise<ApiResponse<AutomaticMessagesMensagem[]>> {
    return this.request<AutomaticMessagesMensagem[]>('/mensagens');
  }

  // Buscar mensagem por ID
  async getMensagemById(id: string): Promise<ApiResponse<AutomaticMessagesMensagem>> {
    return this.request<AutomaticMessagesMensagem>(`/mensagens/${id}`);
  }

  // Criar nova mensagem
  async createMensagem(mensagem: AutomaticMessagesMensagemForm): Promise<ApiResponse<AutomaticMessagesMensagem>> {
    const response = await this.request<AutomaticMessagesMensagem>('/mensagens', {
      method: 'POST',
  ...
  // Atualizar mensagem
  async updateMensagem(id: string, mensagem: Partial<AutomaticMessagesMensagemForm>): Promise<ApiResponse<AutomaticMessagesMensagem>> {
    const response = await this.request<AutomaticMessagesMensagem>(`/mensagens/${id}`, {
      method: 'PUT',
      body: JSON.stringify(mensagem),
    });
  ...
  async searchMensagens(params: {
    ...
    page?: number;
    limit?: number;
  }): Promise<ApiResponse<{ mensagens: AutomaticMessagesMensagem[]; total: number }>> {
    ...
    return this.request<{ mensagens: AutomaticMessagesMensagem[]; total: number }>(endpoint);
  }
  ...
  // Duplicar mensagem
  async duplicarMensagem(id: string): Promise<ApiResponse<AutomaticMessagesMensagem>> {
    const response = await this.request<AutomaticMessagesMensagem>(`/mensagens/${id}/duplicar`, {
      method: 'POST',
    });
  ...
  async reorderMensagens(mensagensOrdenadas: { id: string; ordem: number }[]): Promise<ApiResponse<any>> {
    try {
      // Atualizar cada mensagem individualmente usando PUT /automatic-messages/{id}
      const updatePromises = mensagensOrdenadas.map(mensagem => 
        this.request<AutomaticMessagesMensagem>(`/mensagens/${mensagem.id}`, {
          method: 'PUT',
...
export const automaticMessagesMensagensApi = new AutomaticMessagesMensagensApiService();
export default automaticMessagesMensagensApi;
