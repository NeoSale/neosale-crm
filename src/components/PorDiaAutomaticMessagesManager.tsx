...
import { automaticMessagesApi, DetalheAutomaticMessages } from '../services/automaticMessagesApi';
...
const PorDiaAutomaticMessagesManager: React.FC = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [detalhes, setDetalhes] = useState<DetalheAutomaticMessages[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<DetalheAutomaticMessages | null>(null);
  ...
  const loadDetalhes = async (data?: string) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await automaticMessagesApi.getDetalhesPorData(dateToLoad);
  ...
  const columns: TableColumn<DetalheAutomaticMessages>[] = [
    {
      key: 'nome_lead',
      header: 'Lead',
  ...
  const openModal = (item: DetalheAutomaticMessages) => {
  ...
  const DetailModal = ({ item, isOpen, onClose }: { item: DetalheAutomaticMessages | null; isOpen: boolean; onClose: () => void }) => {
    if (!item) return null;
        ...
        <div className="bg-primary text-white p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold !text-white">Mensagens Automáticas por Dia</h2>
...
export default PorDiaAutomaticMessagesManager;
